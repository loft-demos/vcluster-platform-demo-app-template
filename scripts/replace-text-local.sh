...
Options:
  ...
  --runner-job-image-tag TAG
                          Optional. Defaults to the local git short SHA.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --forgejo-username)
      ...
      shift 2
      ;;
    --runner-job-image-tag)
      RUNNER_JOB_IMAGE_TAG="${2:-}"
      shift 2
      ;;
    --git-base-url)
      GIT_BASE_URL="${2:-}"
...
if [[ -z "$RUNNER_JOB_IMAGE_TAG" ]]; then
...
while IFS= read -r file; do
  ...
  rg -l \
    --hidden \
    '\{REPLACE_REPO_NAME\}|\{REPLACE_ORG_NAME\}|\{REPLACE_VCLUSTER_NAME\}|\{REPLACE_BASE_DOMAIN\}|\{REPLACE_FORGEJO_USERNAME\}|\{REPLACE_RUNNER_JOB_IMAGE_TAG\}|\{REPLACE_GIT_BASE_URL\}|\{REPLACE_GIT_BASE_URL_AUTHED\}|\{REPLACE_GIT_PUBLIC_URL\}|\{REPLACE_IMAGE_REPOSITORY_PREFIX\}|\{REPLACE_OCI_REGISTRY_HOST\}|\{REPLACE_SNAPSHOT_OCI_REPOSITORY\}|\{REPLACE_IMAGE_PULL_SOURCE_SECRET_NAME\}|\{REPLACE_1PASSWORD_VAULT\}|\{REPLACE_KARGO_ADMIN_PASSWORD_HASH\}|\{REPLACE_KARGO_TOKEN_SIGNING_KEY\}|\{REPLACE_KARGO_OIDC_SECRET\}|\{REPLACE_FORGEJO_OIDC_SECRET\}|\{REPLACE_FORGEJO_HOST\}' \
    . \
    "${globs[@]}" \
    --glob '!.git/*'
)
...
echo "[INFO] Runner job image tag: $RUNNER_JOB_IMAGE_TAG"
...
export REPO_NAME ORG_NAME VCLUSTER_NAME BASE_DOMAIN GIT_BASE_URL GIT_BASE_URL_AUTHED GIT_PUBLIC_URL IMAGE_REPOSITORY_PREFIX OCI_REGISTRY_HOST SNAPSHOT_OCI_REPOSITORY IMAGE_PULL_SOURCE_SECRET_NAME ONEPASSWORD_VAULT KARGO_ADMIN_PASSWORD_HASH KARGO_TOKEN_SIGNING_KEY KARGO_OIDC_SECRET FORGEJO_OIDC_SECRET FORGEJO_HOST RUNNER_JOB_IMAGE_TAG FORGEJO_USERNAME_VALUE
...
for file in "${files[@]}"; do
  perl -0pi -e '
    ...
    s/\{REPLACE_RUNNER_JOB_IMAGE_TAG\}/$ENV{RUNNER_JOB_IMAGE_TAG}/g;
